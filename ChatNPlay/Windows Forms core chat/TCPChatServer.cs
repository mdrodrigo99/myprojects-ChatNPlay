...
namespace Windows_Forms_Chat
{
    public class TCPChatServer : TCPChatBase
    {
        ...
        // create a RPSTEAM game, so players can join to existing team or create a new game
        public RPSTeam board = new RPSTeam(null, null);

        public static TCPChatServer createInstance(int port, TextBox chatTextBox, RPSGame game)
        {
        ...
        public void SetupServer()
        {
            ...
            // Create the User table after server setup process complete
            chatTextBox.Text += DatabaseServer.CreateUser() + Environment.NewLine;
        }

        public void SetPlayerSelection(String selection) 
        {
        ...
        public void ReceiveCallback(IAsyncResult AR)
        {
            ...
            else if (text.ToLower() == "!quit")
            {
                if (board.IsPlayerEqual(currentClientSocket))
                {
                    ...
                    SendToClient("nb@p1p2%", currentClientSocket);
                    ...
                    if (board.IsPlayerNotNull(board.GetOtherPlayer(currentClientSocket)))
                    {
                        ...
                        // reset the client board
                        SendToClient("nb@p1p2%", board.GetOtherPlayer(currentClientSocket));
                    }
            ...
            else if (text.ToLower().Equals("!new")) 
            {
                // any of the two players can request for a new game or they can  quit from the game
                // reset server game board
                SetPlayerSelection("");
                ...
                // player 1
                SendToClient("ct@p1p2%", board.GetPlayer1());
            }
            else if (text.ToLower().Contains("mv@p1p2%"))
            {
                ...
                if (board.IsPlayerEqual(currentClientSocket) && board.IsTwoPlayersAvailable())
                {
                    ...
                    // if its player 1
                    if (currentClientSocket.player == PlayerNumber.player1)
                    {
                        UpdateGame(0, text.Remove(0, 8));
                        
                        if (text.Remove(0, 8).ToLower().Equals("rock") || text.Remove(0, 8).ToLower().Equals("paper") || text.Remove(0, 8).ToLower().Equals("scissors"))
                        {
                            SetPlayerSelection(text.Remove(0, 8));
                        }

                        // send move information to current client
                        SendToClient("mv1@p1p2%" + text.Remove(0, 8), currentClientSocket);
                        SendToClient("ct@p1p2%", board.GetPlayer2());
                    }
                    else if (currentClientSocket.player == PlayerNumber.player2)
                    {
                        UpdateGame(1, text.Remove(0, 8));

                        // send move information to current client
                        SendToClient(text, currentClientSocket);

                        // send move information to other player
                        SendToClient("mv2@p1p2%" + text.Remove(0, 8), board.GetPlayer1());

                        SendToClient("mv2@p1p2%" + GetPlayerSelection(), currentClientSocket);

                        if ((game.grid[0] != Choice.blank) && (game.grid[1] != Choice.blank))
                        {
                            currentClientSocket.level += 0.5;
                            if (game.CheckForWin() == State.draw)
                            {
                            ...
                            else if (game.CheckForWin() == State.p1Wins)
                            {
                            ...
                            else if (game.CheckForWin() == State.p2Wins)
                            {
            ...
            else if (text.ToLower() == "!join")
            {
                ...
                if (board.CheckSpaceAvailable(currentClientSocket))
                {
                    ...
                    SendToClient("Your state changed to <playing>. Now you can play the RPS game.", currentClientSocket);
                ...
                {
                    SendToClient("No space available on the RPS game. Please wait!", currentClientSocket);
                }
            }
            else if (text.ToLower() == "!exit") // Client wants to exit gracefully
            {
                ...
                {
                    SendToClient("First, you must exit from the game (type !quit).", currentClientSocket);
                }
                else
                {
                ...
                if(from == null || !from.socket.Equals(c))
                {
                    if (from == null) 
                    {
                        if (str.ToLower().Contains("!mods"))
                        {
                            ...
                            foreach (ClientSocket cl in clientSockets)
                            {
                                if (cl.role == "moderator")
                                {
                                    AddToChat(cl.username + " (" + cl.level + ")");
                                    tmpcnt++;
                                }
                            }
                    ...
                    {
                        byte[] data = Encoding.ASCII.GetBytes("[" + from.username + " (" + from.level + ")" + "] " + str);
                        c.socket.Send(data);
                    }
