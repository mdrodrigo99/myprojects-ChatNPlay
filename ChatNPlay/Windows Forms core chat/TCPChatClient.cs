...
namespace Windows_Forms_Chat
{
    public class TCPChatClient : TCPChatBase
    {
        ...
        public static TCPChatClient CreateInstance(int port, int serverPort, string serverIP, TextBox chatTextBox, RPSGame game)
        {
        ...
        public void ReceiveCallback(IAsyncResult AR)
        {
            ...
            if (text.ToLower().Contains("mv1@p1p2%") && !text.Remove(0, 9).ToLower().Contains("mv1@p1p2%"))
            {
                // update the client's board uisng move information
                UpdateGame(0, text.Remove(0, 9));
            }
            else if (text.ToLower().Contains("mv2@p1p2%") && !text.Remove(0, 9).ToLower().Contains("mv2@p1p2%"))
            {
                // update the client's board uisng move information
                UpdateGame(1, text.Remove(0, 9));
            }
            ...
            else if (text.ToLower() == "nb@p1p2%")
            {
                ...
                AddToChat("Your state changed to <chatting>. You can now chat but cannot play RPS games.");
                AddToChat("New players can join to the game.");
            }
            else if (text.ToLower() == "rb@p1p2%")
            {
                ...
                //currentClientSocket.player = PlayerNumber.nonplayer;
                //AddToChat("Your state changed to <chatting>. You can now chat but cannot play RPS games.");
                AddToChat("New game starts now.");
            }
            else if (text.ToLower().Contains("mv@p1p2%") && !text.Remove(0, 8).ToLower().Contains("mv@p1p2%"))
            { 
